...
#sudo apt install screenfetch -y; sudo apt install simple-scan -y; sudo apt install gnome-tweaks -y; sudo apt install java-latest-openjdk -y; sudo apt install btop -y; sudo apt install flatpak -y; sudo apt install gnome-software-plugin-flatpak -y; sudo apt install kitty -y; sudo apt install boxes -y; sudo add-apt-repository ppa:cappelikan/ppa; sudo apt search kernel mainline; flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo;
